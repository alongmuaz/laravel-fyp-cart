...
class Cart {
	 ...
	 * @param float  	    $profit   Profit of one item
	 * @param array  	    $options  Array of additional options, such as 'size' or 'color'
	 */
	public function add($id, $name = null, $qty = null, $price = null, $profit = null, array $options = [])
	{
		...
		if(is_array($id))
		{
			...
			if($this->is_multi($id))
			{
				...
				foreach($id as $item)
				{
					...
					$this->addRow($item['id'], $item['name'], $item['qty'], $item['price'], $item['profit'], $options);
				}
			...
			$result = $this->addRow($id['id'], $id['name'], $id['qty'], $id['price'], $id['profit'], $options);
		...
		// Fire the cart.add event
		$this->event->fire('cart.add', compact('id', 'name', 'qty', 'price', 'profit', 'options'));

		$result = $this->addRow($id, $name, $qty, $price, $profit, $options);

		// Fire the cart.added event
		$this->event->fire('cart.added', compact('id', 'name', 'qty', 'price', 'profit', 'options'));
	 ...
	 * @param array   $options  Array of additional options, such as 'size' or 'color'
	 */
	protected function addRow($id, $name, $qty, $price, $profit, array $options = [])
	{
		...
		{
			$cart = $this->createRow($rowId, $id, $name, $qty, $price, $profit, $options);
		}
	...
	protected function updateRow($rowId, $attributes)
	{
		...
		if( ! is_null(array_keys($attributes, ['qty', 'price', 'profit'])))
		{
			$row->put('subtotal', $row->qty * $row->price);
			$row->put('taxsubtotal', $row->qty * $row->profit);
		}
	 ...
	 * @return Gloudemans\Shoppingcart\CartCollection
	 */
	protected function createRow($rowId, $id, $name, $qty, $price, $profit, $options)
	{
		$cart = $this->getContent();
